...
void intercept_packet(enum packet_type type, void* packet, char* visited,int* count) {
  ...
  if(type==PACKET_PROCESSING_STARTED) {
    ...
    *count+=1;
    ...
    //terrain = tile_terrain(&tile);
    int x = index_to_map_pos_x(tile_index(&tile));
    int y = index_to_map_pos_y(tile_index(&tile));
    struct map_index* vec = tile_to_vec(&tile);
    update_map(x,y,vec);
    if (tile.index < 1<<20 && visited[tile.index]<128){
      visited[tile.index] += 1;
      printf("\nTILE_INFO PACKET (15):\ntile_id: %d\n# packets of tile recieved: %d\n(x,y) coords: (%d,%d)",tile_index(&tile),visited[tile.index],x,y);
    }
    else {
   ...
   if (*count%10==0) {
     ...
     request_unit_non_action_move(&unit, tile);
   }
